...
samtools faidx $REFERENCE || { echo "[ERROR] Échec de l'indexation du génome !"; exit 1; }
...
for QUERY in "${QUERY_FILES[@]}"; do
    ...
    awk '{print $NF "\t" $1 "\t.\t" $2 "\t" $3 "\t.\tPASS\t."}' $PREFIX.snps >> $PREFIX.vcf || { echo "[ERROR] Échec de la conversion VCF pour $QUERY"; continue; }
